...
git clone https://gitlab.cee.redhat.com/dataverse/ai/mcp-servers/template-mcp-server.git "$NEW_PROJECT_NAME"
if [ $? -ne 0 ]; then
    print_error "Failed to clone template repository"
    print_warning "Make sure you have access to: https://gitlab.cee.redhat.com/dataverse/ai/mcp-servers/template-mcp-server.git"
    exit 1
fi
cd "$NEW_PROJECT_NAME"
...
echo ""
echo "  3. Install Red Hat certificates:"
echo "     wget https://certs.corp.redhat.com/certs/Current-IT-Root-CAs.pem \\"
echo "       && cat Current-IT-Root-CAs.pem >> \`python -m certifi\`"
echo ""
